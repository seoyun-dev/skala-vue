...
* **날씨 배경 사진** — Unsplash API 키를 등록하면 화면 배경이 실제 날씨(맑음 / 흐림 / 비 / 천둥 / 눈)에 맞는 사진으로 바뀝니다. 천둥(Thunderstorm)은 평범한 비와 헷갈리지 않도록 번개 사진이 나오는 별도 테마로 분리했습니다. 키가 없으면 자동으로 CSS 그라디언트 테마로 대체됩니다.
...
Vite의 `VITE_*` 환경 변수는 **빌드 시점**에 결과물(JS 번들)에 박혀 들어갑니다. 로컬 `.env`는
`npm run dev`/`npm run build`에만 쓰이고, GitHub Actions 빌드에는 전혀 영향을 주지 않습니다 —
배포된 사이트에서 API 키가 필요한 기능(날씨 배경 사진 등)을 쓰려면, 레포 **Settings → Secrets and
variables → Actions**에 `.env`와 같은 이름(`VITE_OPENWEATHER_API_KEY`, `VITE_UNSPLASH_ACCESS_KEY`)으로
secret을 등록하고, `deploy.yml`의 빌드 스텝 `env:`에도 `${{ secrets.이름 }}`으로 매핑해 둬야 합니다.
Secret만 등록하고 워크플로우의 `env:`에 매핑을 빠뜨리면, 빌드된 코드에는 그냥 빈 값이 들어가서 조용히
그라디언트 폴백으로 넘어갑니다(에러가 나지 않아 원인을 알기 어렵습니다).
